...
public class BoardRepositoryTests {
    ...
    public void testInsert() {
        // 1부터 100까지 100개의 임의의 board 객체를 생성
    ...
    // select 기능 테스트(행 1개만 가져오기)
    @Test
    public void testSelect() {
        Optional<Board> result = boardRepository.findById(10L);  // bno가 10인 행 가져오기
        Board board = result.orElseThrow();

        log.info(board);
    }

    // select 기능 테스트(행 전체 가져오기)
    @Test
    ...
    public void testUpdate() {
        ...
        boardRepository.save(board);    // board라는 객체에 id값이 있기 때문에 해당 save는 update sql이 실행
    }
    ...
    public void testDelete2()    {
        Optional<Board> result = boardRepository.findById(9L);  // bno가 10인 행 가져오기
        Board board = result.orElseThrow();
    ...
    public void queryMethodTest2() {
        List<Board> boardList = boardRepository.findByWriterIn(Arrays.asList("user9", "user3"));
...
//        List<Board> boardList = boardRepository.findByWriterEndingWith("1");
//
//        boardList.forEach(board -> log.info(board));
//    }
...
//                = PageRequest.of(0, 10, Sort.by("bno").descending());
//        Page<Board> boardPage
//                = boardRepository.findDistinctByWriter("user1", pageable);
//
//        boardPage.forEach(board -> log.info(board));
//    }
    ...
    public void queryMethodAnnotationTest2() {
        List<Board> boardList = boardRepository.findFromTitleWriter("title....1", "user1");
    ...
    public void queryMethodAnnotationTest3() {
        List<Board> boardList = boardRepository.findFromWriters(Arrays.asList("user9", "user3"));
     ...
     */
    @Test
    //@Transactional      // OneToMany에서 N+1 문제 발생
...
//        Optional<Board> result = boardRepository.findById(1L);      // LAZY로 fetch
    ...
    public void testModifyImages() {
        ...
        // 기존의 첨부 파일들과 연계된 게시물 번호 속성을 null로 세팅(orphanRemoval가 true일 경우 적용)
        board.clearImages();
    ...
    public void testRemoveAll() {
        ...
        // 댓글 삭제(@ManyToOne에는 orphanRemoval 속성 없음 -> 별도로 댓글 삭제 jpa 호출)
    ...
    public void testInsertAll() {
        ...
        for(int i = 1; i <= 100; i++) {
            ...
            for(int j = 0; j < 3; j++) {
                if(i % 5 == 0) {        // 게시물이 5의 배수인 것들만 첨부 파일 데이터 추가
